...
class ReconNet(nn.Module):
    def __init__(self, out_channels=None):
        ...
        self.middle = nn.Sequential(
                nn.ConvTranspose3d(480, 240, kernel_size=3, stride=(1,2,2), 
                                   padding=1, output_padding=(0,1,1), bias=False),
                nn.BatchNorm3d(240),
                nn.ReLU(inplace=True),
                
                nn.ConvTranspose3d(240, 96, kernel_size=3, stride=(1,2,2), 
                                   padding=1, output_padding=(0,1,1), bias=False),
                nn.BatchNorm3d(96),
                nn.ReLU(inplace=True),

                nn.ConvTranspose3d(96, out_channels, kernel_size=3, stride=2, 
...
class InceptionI3d(nn.Module):
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True, size=None,
        ...
        if size == '96x96':
            self.param = {
                'Conv3d_1a_7x7': {'stride': (1, 1, 1)},
                'MaxPool3d_5a_2x2': {'stride': (1, 2, 2)}
                }
        elif size == '224x224':
            self.param = {
                'Conv3d_1a_7x7': {'stride': (1, 2, 2)},
        ...
        self.end_points[end_point] = Unit3D(
            ...
            output_channels = 64, 
            kernel_shape = [7, 7, 7],
            stride = self.param['Conv3d_1a_7x7']['stride'],
            padding = (3,3,3),  
            name=name+end_point,
            )
        ...
        self.end_points[end_point] = MaxPool3dSamePadding(
            kernel_size = [2, 2, 2], 
            stride = self.param['MaxPool3d_5a_2x2']['stride'],
            padding = 0,
            )
        if self._final_endpoint == end_point: return
    ...
    def forward(self, x):
        for end_point in self.VALID_ENDPOINTS:
            if end_point in self.end_points:
                ...
                if end_point == 'MaxPool3d_4a_3x3': x2 = x
...
class BranchI3D(nn.Module):
    def __init__(self, out_channels):
        super(BranchI3D, self).__init__()
        self.main = InceptionI3d(400, in_channels=3)
        self.main.load_state_dict(torch.load('./pretrained/rgb_imagenet.pt'))
        
        self.recon = ReconNet(out_channels=out_channels)
