...
class InceptionI3d(nn.Module):
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True, size=None,
                 ...
                 in_channels=3, dropout_keep_prob=0.5, first_stride=(1,2,2)):
    ...
    def forward(self, x):
        ...
        return x.squeeze(2).squeeze(2).squeeze(2)
...
class MultipleI3D(nn.Module):
    def __init__(self):
        super(MultipleI3D, self).__init__()
        self.main01 = InceptionI3d(400, in_channels=3)
        self.main02 = InceptionI3d(400, in_channels=2)
        self.fc = nn.Linear(1024*2, 30)
        ...
        self.main01.load_state_dict(torch.load('./pretrained/rgb_imagenet.pt'))
  
        
    def forward(self, x1, x2):        
        vec01 = self.main01(x1)  
        vec02 = self.main02(x2) 

        out = torch.cat((vec01, vec02), 1)
        out = self.fc(out)
